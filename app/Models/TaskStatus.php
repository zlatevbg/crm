...
class TaskStatus extends Model
{
    ...
    {
        return Status::where('parent', 6)->orderBy('name')->pluck('name', 'id');
    }
    ...
    {
        $data = $request->all();
        $data['task_id'] = $api->model->_parent->id;
    ...
    public function dColumns()
    {
        return [
            /*[
                ...
                'checkbox' => true,
                'order' => false,
                'hidden' => (!Auth::user()->can('Select Datatable Rows') || (!Auth::user()->can('Create: Task Status') && !Auth::user()->can('Edit: Task Status') && !Auth::user()->can('Delete: Task Status'))),
            ],*/
            [
                'id' => 'created',
    ...
    public function dButtons($api)
    {
        return [/*
            'create' => [
                ...
                'visible' => Auth::user()->can('Create: Task Status'),
            ],
            'delete' => [
                ...
                'visible' => Auth::user()->can('Delete: Task Status'),
            ],
        */];
    }
    ...
    public function dData($api)
    {
        ...
        $data = $this->withTrashed()->selectRaw($table . '.id, ' . $table . '.created_at, CONCAT(users.first_name, " ", users.last_name) as user, statuses.name as status')
            ...
            ->where('task_id', $api->model->_parent->id)
