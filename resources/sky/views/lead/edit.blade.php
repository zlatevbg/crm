...
@section('content')
    <form method="POST" action="{{ Helper::route('api.update', $api->path) }}" accept-charset="UTF-8" id="edit-form" data-ajax novalidate>
        ...
        <div class="form-group">
            ...
            <input id="input-email" class="form-control" autofocus placeholder="@lang('placeholders.email')" name="email" type="email" value="{{ $api->model->email }}">
        </div>

        <div class="form-group">
            <label for="input-sources">@lang('labels.source')</label>
            <select multiple id="input-sources" class="form-control" name="sources[]">
                @php $selectedSources = $api->model->sources->pluck('id')->all(); @endphp
                @foreach ($api->model->selectSources() as $key => $value)
                    <option value="{{ $key }}" {!! in_array($key, $selectedSources) ? 'selected="selected"' : '' !!}>{{ $value }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input id="input-name" class="form-control" placeholder="@lang('placeholders.name')" name="name" type="text" value="{{ $api->model->name }}">
        </div>
